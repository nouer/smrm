...
**キャンセル時の確認**: 「キャンセル」ボタンをタップするか、フォーム外の背景をタップすると、「編集中の内容が失われます。閉じますか？」という確認ダイアログが表示されます。誤操作による入力内容の喪失を防ぎます。
...
**背景タップで閉じる**: 詳細画面の外側（背景部分）をタップしても閉じることができます。
...
- **施術日時**: デフォルトで現在日時が設定されます。変更も可能です
...
直近の記録には「詳細」「編集」「削除」の3つのボタンがあります:

- **詳細**: 施術記録の内容を読み取り専用で確認できます
- **編集**: 施術記録の内容を変更できます。編集中に「キャンセル」ボタンをタップするか、フォーム外の背景をタップすると、確認ダイアログが表示されます
- **削除**: 施術記録を削除できます（確認ダイアログあり）

### 4.6 施術記録の詳細表示

「詳細」ボタンをクリックすると、施術記録の全項目を読み取り専用のオーバーレイで確認できます。施術日時、メニュー、主訴、所見、施術内容、施術後メモ、体調レベル、写真が一覧表示されます。

オーバーレイ下部の「編集」ボタンから、そのまま編集画面に遷移することもできます。背景部分をタップしても閉じることができます。
...
![施術履歴タイムライン](images/08_history_timeline.png)
...
タイムラインの記録を展開すると、「詳細」「編集」「削除」のアクションボタンが表示されます。詳細ボタンで記録の全項目を確認、編集ボタンで内容を変更、削除ボタンで記録を削除できます。
