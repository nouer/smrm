...
| メールアドレス | string | | email形式（HTML `type="email"` によるブラウザネイティブバリデーション） |
| 住所 | string | | |
| 職業 | string | | |
| 紹介元 | string | | |
| 利用動機 | string | | |
...
- 写真未登録時の顧客イニシャル表示（顧客カード）— 氏名をスペースで分割し先頭文字を最大2文字取得
...
| 2 | 連絡先 | 電話番号、メール、住所 |
| 3 | 利用情報 | 職業、紹介元、利用動機、初回施術日、担当施術者 |
| 4 | 備考 | 備考テキスト |
...
- 動的な行追加・削除
- 施術記録タブでのアレルギー警告表示（アレルギー情報がある顧客を選択した際に、顧客情報バー直下に赤い警告バーでアレルゲン名・重症度を表示）
- 顧客登録フォーム内で管理
...
顧客コード、ふりがな、電話番号、メールアドレス、住所、職業、初回施術日、担当施術者、備考、アレルギー情報、既往歴、写真添付
...
| 項目 | 値 |
|------|-----|
| 通知条件 | 前回エクスポートから指定日数が経過した場合 |
| 選択可能間隔 | 3日 / 7日（デフォルト）/ 14日 / 30日 |
| 表示場所 | ヘッダー直下のバナー |
| 記録先 | LocalStorage（最終エクスポート時刻） |

#### 機能

- アプリ起動時に前回エクスポートからの経過日数をチェック
- 設定日数を超過している場合、エクスポート推奨バナーを表示
- チェックボックスで通知のオン/オフ切り替え
- 「この通知を表示しない」で個別の非表示が可能
...
| 項目 | 値 |
|------|-----|
| 情報元 | `/notify.html` から取得 |
| 未読判定 | お知らせ内容のSHA-256ハッシュで既読/未読を判定 |
| 表示場所 | ヘッダー直下のバナー |
| 記録先 | LocalStorage（既読ハッシュ） |
...
- オーバーレイ操作: 読み取り専用オーバーレイは背景タップで即座に閉じる。編集系オーバーレイはキャンセル・背景タップ時に確認ダイアログを表示し、誤操作による入力内容の喪失を防止する
- ボタンレイアウト: アクションボタン（詳細・編集・削除）は右寄せ配置。削除ボタンは他のボタンとの間隔を広くし、誤操作を防止する

### 4.4 可用性

- オフライン環境でも全機能が利用可能であること
  - ただし以下の機能はネットワーク接続を前提とし、オフライン時は無効化またはスキップされる:
    - お知らせ通知の取得（`/notify.html` の取得に失敗した場合は通知をスキップ）
    - Service Worker の更新確認（オフライン時は「更新を確認」ボタンが機能しない）
    - サンプルデータのインポート（オフライン時はエラーメッセージを表示）
