...
| 3 | `history` | 履歴 | 施術履歴タイムライン（顧客選択が前提） |
...
[設定タブ]
  ...
  ├── 表示設定
  ├── 画像圧縮設定
  ├── エクスポート通知設定（指定日数経過時にバナー表示）
  ├── お知らせ通知（開発元からのお知らせ表示設定）
  ├── アプリ情報
  └── サンプルデータインポート
```
...
| モーダル | トリガー | 内容 |
|---------|---------|------|
| 顧客登録/編集モーダル | 新規登録ボタン / 編集ボタン | 顧客フォーム全項目。キャンセル・背景クリックで確認ダイアログ表示 |
| 施術記録編集モーダル | タイムライン内の編集ボタン | 施術記録全項目。キャンセル・背景クリックで確認ダイアログ表示 |
| 確認ダイアログ | 削除操作・編集キャンセル・背景クリック | タイトル・メッセージ・確認ボタン（z-index: 210で編集オーバーレイの上に表示） |
| 顧客詳細オーバーレイ | 詳細ボタン / 顧客情報バークリック | 顧客情報の読み取り専用表示（基本情報・連絡先・利用情報・備考・アレルギー・既往歴・写真・メタ情報の8セクション）、閉じる・編集ボタン、背景クリックで即閉じ |
| 施術記録詳細オーバーレイ | 直近記録/タイムラインの「詳細」ボタン | 施術記録の読み取り専用表示（日時・メニュー・主訴・所見・施術内容・施術後メモ・体調レベル・写真）、閉じる・編集ボタン、背景クリックで即閉じ |
