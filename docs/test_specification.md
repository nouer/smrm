...
| テストID | テスト名 | 操作 | 期待結果 |
|---------|---------|------|---------|
| E2E-050 | 顧客詳細オーバーレイ背景クリックで閉じる | 顧客詳細表示 → オーバーレイ背景クリック | `#customer-detail-overlay` から `.show` が除去される |
| E2E-051 | 施術記録詳細オーバーレイ背景クリックで閉じる | 施術記録詳細表示 → オーバーレイ背景クリック | `#record-detail-overlay` から `.show` が除去される |
| E2E-052 | 顧客編集キャンセルで確認ダイアログ表示 | 顧客編集フォーム表示 → キャンセルボタン | `#confirm-overlay` に `.show` が付与され、「編集中の内容が失われます」メッセージ表示 |
| E2E-053 | 顧客編集背景クリックで確認ダイアログ表示 | 顧客編集フォーム表示 → オーバーレイ背景クリック | `#confirm-overlay` に `.show` が付与される |
| E2E-054 | 施術記録編集キャンセルで確認ダイアログ表示 | 施術記録編集フォーム表示 → キャンセルボタン | `#confirm-overlay` に `.show` が付与される |
| E2E-055 | 確認ダイアログでキャンセル→編集に戻る | 確認ダイアログ表示 → キャンセルボタン | 確認ダイアログが閉じ、編集オーバーレイは開いたまま |
| E2E-056 | 確認ダイアログでOK→オーバーレイ閉じる | 確認ダイアログ表示 → 「閉じる」ボタン | 確認ダイアログと編集オーバーレイの両方が閉じる |
